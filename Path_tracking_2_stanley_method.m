...
for t=0:T:final_time
    ...
    v_car = [Z(1) - WP1(1); Z(2) - WP1(2); 0];
    v_wp = [WP2(1) - WP1(1); WP2(2) - WP1(2); 0];
    v_car_wp_cross = cross(v_car, v_wp);
    e_l=sqrt((x_d-Z(1))^2+(y_d-Z(2))^2)*sign(v_car_wp_cross(3));
    if abs(e_l)>20
...
%     yaw_d=atan2((y_pv-Z(2)),(x_pv-Z(1)));    % desired heading angle
    ...
    e_y_k = 2;
    e_y_input = e_y + atan2(e_y_k*e_l, abs(Z(3))+2);
    ...
    %M_d=0;
    M_c=I_z*(K_yd*e_yd+K_yp*e_y_input); 
